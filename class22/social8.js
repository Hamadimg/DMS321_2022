...
async function socialSearch(req, res) {
    ...
    let collection = db.collection("users");
    let query = { screenname: new RegExp(`${params.searchterm}`,'i') };        /* query for people who match the search term */
    collection.find(query).toArray(async function (err,result) {
        if (err) { console.log(err); return res.sendStatus(500); }
        if (params.ajax == 1)
            sendSearchResultsAjax(req, res, result, following);
        else
            sendSearchResultsHtml(req, res, result, following);
        });
    }


function sendSearchResultsHtml(req, res, result, following)
    {
    let users = [];
    for (let i=0; i < result.length; i++) {                                  /* save the results in an array for the EJS template */
        if (result[i]._id == req.session.user._id) continue;                 /* don't include myself in the results */
...
function sendSearchResultsAjax(req, res, result, following)
    {
    let output = '<table>';
    for (let i=0; i < result.length; i++) {                                  /* save the results in a single string */
        if (result[i]._id == req.session.user._id) continue;                 /* don't include myself in the results */
        output += `<tr><td><a href="/social/profile/${result[i]._id}">${result[i].screenname}</a></td>`;
        if (following.includes(result[i]._id.toString()))
            output += `<td><a href="/social/unfollow/${result[i]._id}">unfollow</a></td>`;
        else
            output += `<td><a href="/social/follow/${result[i]._id}">follow</a></td></tr>`;
        }
    output += '</table>';
    res.send(output);
    }
...
router.get('/social/search', socialSearch);
