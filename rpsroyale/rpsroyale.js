...
async function home(req, res) {
    if (!req.session.rpsr_user) { res.redirect('loginpage'); }
    let db = await getDb();
...
function scoreboard(req, res) {
    res.render('rpsroyale/scoreboard', { user: req.session.rpsr_user });
    }


function newplay(req, res) {
    if (!req.session.rpsr_user) { res.redirect('loginpage'); }
    res.render('rpsroyale/newplay', { user: req.session.rpsr_user });
    }
...
async function match(req, res) {
    ...
    collection.find(query).toArray(function (err, result) {
        if (err) { console.log(err); return res.sendStatus(500); }
        res.render('rpsroyale/match', { user: req.session.rpsr_user, plays: result });
        });
    }
...
async function makematch(req,res) {
    ...
    collection.findOneAndDelete(query).then(function (result) {
        ...
        if (!play) { return res.render('rpsroyale/matchfailed'); }
        ...
        res.render('rpsroyale/matchresult', { user: req.session.rpsr_user, match: match, matchRes: matchRes });
...
function loginPage(req, res) {
    res.render('rpsroyale/loginpage');
    }
...
async function newAccount(req, res) {
    ...
    if (!errors.isEmpty()) { return res.render('rpsroyale/error', { errors: errors.array() }); }
    let db = await getDb();
    let collection = db.collection("users");
    let query = { email: new RegExp(`^${req.body.username}$`,'i') };
...
function loginError(req, res) {
    res.render('rpsroyale/loginerror');
    }


function newAccountError(req, res) {
    res.render('rpsroyale/newaccounterror');
    }
